...
class BaseProcessor(ABC):
    ...
    def invoke_llm_with_trace(
        ...
        return self.llm_strategy.invoke(prompts=prompts, metadata_=metadata_)
    ...
    def raise_error_tracing(self, e):
        """!
        Raises an error under a trace within the run to mark the failure in Langfuse.
    ...
    def _execute_code_remotely(self, code, validation_workspace):
        ...
        remote_path = os.getenv('REMOTE_EXEC_PATH', '/home/XXXX-5/tinyml_autopilot/tmp')
        remote_env_activate = os.getenv('REMOTE_PYTHON_ENV', 'source ~/tinyml_autopilot/tinyml-env/bin/activate')
        remote_python = os.getenv('REMOTE_PYTHON_EXECUTABLE', '/home/XXXX-5/tinyml_autopilot/tinyml-env/bin/python')      
