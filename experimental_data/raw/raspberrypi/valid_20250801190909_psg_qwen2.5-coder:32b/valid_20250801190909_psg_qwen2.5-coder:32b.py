...
while cap.isOpened():
    ...
    # Assuming the model outputs a classification score for each class in labels
    scores = np.squeeze(output_data)
    detected_class = labels[np.argmax(scores)]
    confidence_score = np.max(scores)

    # Draw detection result on frame
    cv2.putText(frame, f'{detected_class}: {confidence_score:.2f}', (10, 30), 
